...
class AccidentController extends Controller
{
    private $validationMessages = [
        ...
        'status.in' => 'Статус должен быть одним из: pending, approved, rejected, paid',
    ];
    ...
    public function store(Request $request, $policyId)
    {
        try {
            ...
            if (!$policy) {
                return response()->json(['message' => 'Активный полис не найден'], 404);
            }
            ...
            if ($validated['accident_date'] > $policy->end_date) {
                return response()->json(['message' => 'Дата ДТП не может быть позже даты окончания действия полиса'], 422);
            }
            ...
            return response()->json([
                'message' => 'Информация о ДТП успешно отправлена',
            ...
            return response()->json(['message' => 'Ошибка при создании заявления'], 500);
        }
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            if (isset($validated['status']) && $validated['status'] === 'paid') {
                ...
                if ($currentFault) {
                    return response()->json(['message' => 'Нельзя оплатить ДТП, где клиент виновник'], 422);
                }
            }
            ...
            return response()->json([
                'message' => 'Информация о ДТП обновлена',
